...
export let activateShowAddObjectPanel = function( commandId, location, data, ctx, objectSetSource ) {
    ...
    if (selected.props.sci0IsSemicomponent.dbValues[0] === '1') {
...
var prepareDataForCreateObjectSOA = function( data, ctx ) {
    ...
    var primaryObjInfo = null;

    if ( ctx.selected.modelType.typeHierarchyArray.indexOf( 'Awb0ConditionalElement' ) > -1  )
    {
        let tmpSelected = _cdmSvc.getObject( ctx.selected.uid );
        primaryObjInfo = _cdmSvc.getObject( tmpSelected.props.awb0UnderlyingObject.dbValues[0] );
    } else {
        primaryObjInfo = _cdmSvc.getObject( ctx.selected.uid );
    }
