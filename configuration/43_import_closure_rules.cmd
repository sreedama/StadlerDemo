...
for %%f in (closure_rules\*.xml) do (
    echo -----------------------------------------------------------------------
    echo tcxml_import.exe %CREDENTIALS_TMP% -file="%%f" -scope_rules -scope_rules_mode=overwrite
    echo.
	tcxml_import.exe %CREDENTIALS% -file="%%f" -scope_rules -scope_rules_mode=overwrite
    echo -----------------------------------------------------------------------  
)
