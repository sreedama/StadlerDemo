...
echo Starting clearlocks process %CURRENT_DATE% %CURRENT_TIME%>>%LOG_FILE%
echo.>>%LOG_FILE%
echo Current user sessions>>%LOG_FILE%
REM list current users in log file
list_users>>%LOG_FILE%
echo.>>%LOG_FILE%
echo Clearlocks result>>%LOG_FILE%
clearlocks -verbose>>%LOG_FILE%
echo.>>%LOG_FILE%
echo Clearlocks process finished at %date% %time%>>%LOG_FILE%

echo Clearlocks process finished at %date% %time%
